CREATE TABLE LOAISANH (
	...
	VALUES 
		('Võ Vũ Trường Giang', 'Trần Thị Nhã Phương', '0946139641', '2024-01-05', '2024-01-13', '001', '001', 5000000, 15, 7, 1530000, 33660000),
		('Huỳnh Trấn Thành', 'Hari Won', '0980159392', '2024-01-08', '2024-02-13', '001', '002', 8000000, 21, 7, 1350000, 37800000),
		('Ông Cao Thắng', 'Mai Hồng Ngọc', '0969160756', '2024-01-03', '2024-01-14', '002', '001', 12000000, 16, 6, 1610000, 35420000),
		('Kim Aron Lý', 'Hồ Thị Ngọc Hà', '0944986100', '2024-02-20', '2024-03-02', '004', '002', 10000000, 26, 4, 1560000, 46800000),
		('Lê Công Vinh', 'Trần Thị Thủy Tiên', '0976408712', '2024-01-07', '2024-02-12', '005', '001', 9000000, 30, 5, 1790000, 62650000),
		('Hoàng Kim Uyên', 'Võ Nguyễn Lan Trinh', '0972223639', '2024-02-25', '2024-03-03', '006', '002', 12000000, 16, 2, 1980000, 35640000),
		('Hứa Văn Đạt', 'Lâm Vĩ Dạ', '0975680505', '2024-01-1', '2024-01-07', '002', '001', 12000000, 35, 7, 1400000, 58800000),
	...
	INSERT INTO HOADON (SoPhieuDatTiec, NgaylapHoaDon, NgayThanhToan, TongTienDichVu, TongTienHoaDon, PhiThanhToanTre ,ConLai, TinhTrang)
	VALUES ( '1', '2024-01-13', '2024-01-13', 1050000, 34710000,0, 29710000, 'Chưa thanh toán'),
		( '2', '2024-02-13', '2024-02-13', 650000, 38450000,0, 30450000, 'Chưa thanh toán'),
		( '3', '2024-01-14', '2024-01-14', 2250000, 37670000,0, 25670000,'Chưa thanh toán'),
		( '4', '2024-03-02', '2024-03-02', 700000, 47500000,0, 37500000, 'Chưa thanh toán'),
		( '5', '2024-02-12', '2024-02-12', 1700000, 64350000,0, 55350000, 'Chưa thanh toán'),
		( '6', '2024-03-03', '2024-03-03', 1300000, 36940000,0, 24940000, 'Chưa thanh toán'),
		( '7', '2024-01-07', '2024-01-07', 5600000, 64400000,0, 52400000, 'Chưa thanh toán'),
		( '8', '2024-08-07', '2024-08-07', 600000, 42040000,0, 35040000, 'Chưa thanh toán'),
		( '9', '2024-08-25', '2024-08-25', 1900000, 55150000,0, 47150000, 'Chưa thanh toán'),
		( '10', '2024-05-20', '2024-05-21', 2300000, 80000000,0, 30000000, 'Đã thanh toán');
	...
	VALUES 
		...
		('user', 'user', 'User', 2, 'user@gmail.com', '0797283553'),
		('admin', 'admin', 'Admin', 0, 'admin@gmail.com', '0797886653'),
		('staff', 'staff', 'Staff', 1, 'staffg@gmail.com', '0997883553');

CREATE OR REPLACE FUNCTION trg_KiemTraSanhCa_BanToiDa()
RETURNS TRIGGER AS $$
DECLARE
    ma_sanh VARCHAR(10);
    ma_ca VARCHAR(10);
    ngay_dai_tiec DATE;
    so_luong_ban INT;
    so_luong_ban_toi_da INT;
BEGIN
    -- Lấy thông tin từ bản ghi được chèn vào
    ma_sanh := NEW.MaSanh;
    ma_ca := NEW.MaCa;
    ngay_dai_tiec := NEW.NgayDaiTiec;
    so_luong_ban := NEW.SoLuongBan;

    -- Kiểm tra tình trạng của sảnh và ca vào ngày đã chọn
    IF EXISTS (
        SELECT 1
        FROM PHIEUDATTIEC
        WHERE MaSanh = ma_sanh AND MaCa = ma_ca AND NgayDaiTiec = ngay_dai_tiec
    ) THEN
        RAISE EXCEPTION 'Sảnh và ca đã được đặt vào ngày này. Vui lòng chọn sảnh hoặc ca khác.';
    END IF;

    -- Kiểm tra số lượng bàn tối đa của sảnh
    SELECT SoLuongBanToiDa INTO so_luong_ban_toi_da
    FROM SANH
    WHERE MaSanh = ma_sanh;

    IF so_luong_ban > so_luong_ban_toi_da THEN
        RAISE EXCEPTION 'Số lượng bàn nhập vào vượt quá số lượng bàn tối đa của sảnh.';
    END IF;

    -- Nếu không có lỗi, chèn bản ghi vào bảng PHIEUDATTIEC
    INSERT INTO PHIEUDATTIEC (TenChuRe, TenCoDau, SDT, NgayDatTiec, NgayDaiTiec, MaSanh, MaCa, TienDatCoc, SoLuongBan, SoBanDuTru, DonGiaBan, TongTienBan)
    VALUES (NEW.TenChuRe, NEW.TenCoDau, NEW.SDT, NEW.NgayDatTiec, NEW.NgayDaiTiec, NEW.MaSanh, NEW.MaCa, NEW.TienDatCoc, NEW.SoLuongBan, NEW.SoBanDuTru, NEW.DonGiaBan, NEW.TongTienBan);
